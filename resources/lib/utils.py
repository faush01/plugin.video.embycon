...
def getArt(item, server):
    ...
    if not art['thumb']:
        art['thumb'] = downloadUtils.getArtwork(item, "Primary", parent=True, server=server)
    ...
    if not art['discart']:
        art['discart'] = downloadUtils.getArtwork(item, "Disc", parent=True, server=server)
